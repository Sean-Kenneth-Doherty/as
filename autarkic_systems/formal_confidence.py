...
def _validate_target(
    ...
    if "substitution_graph_correctness_cases" in target.configuration:
        results.extend(_validate_substitution_graph_correctness_cases(target))
...
def _validate_substitution_graph_formula(
    target: FormalConfidenceTarget,
) -> list[FormalConfidenceValidation]:
    subject = f"{target.target_id}.substitution_graph_formula"
    target_path = target.configuration["substitution_graph_formula"]
    try:
        candidates = load_substitution_graph_formula_candidates(target_path)
        report = validate_substitution_graph_formula_candidates(candidates)
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        return [
            _rejected(
                subject,
                "substitution graph formula rejected: " + str(exc),
            )
        ]
    if report.accepted:
        return [_accepted(subject, "substitution graph formula accepted")]
    return [
        _rejected(
            subject,
            "substitution graph formula rejected: "
            + _joined_or_none(report.failed_subjects),
        )
    ]
...
def _validate_substitution_graph_correctness_cases(
    target: FormalConfidenceTarget,
) -> list[FormalConfidenceValidation]:
    subject = f"{target.target_id}.substitution_graph_correctness_cases"
    target_path = target.configuration["substitution_graph_correctness_cases"]
    try:
        cases = load_substitution_graph_correctness_cases(target_path)
        report = validate_substitution_graph_correctness_cases(cases)
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        return [
            _rejected(
                subject,
                "substitution graph correctness cases rejected: " + str(exc),
            )
        ]
    if report.accepted:
        return [_accepted(subject, "substitution graph correctness cases accepted")]
    return [
        _rejected(
            subject,
            "substitution graph correctness cases rejected: "
            + _joined_or_none(report.failed_subjects),
        )
    ]
...
def _validate_fixed_point_construction_cases(
    target: FormalConfidenceTarget,
) -> list[FormalConfidenceValidation]:
    subject = f"{target.target_id}.fixed_point_construction_cases"
    case_path = target.configuration["fixed_point_construction_cases"]
    try:
        cases = load_fixed_point_construction_cases(case_path)
        report = validate_fixed_point_construction_cases(cases)
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        return [
            _rejected(
                subject,
                "fixed-point construction cases rejected: " + str(exc),
            )
        ]
    if report.accepted:
        return [_accepted(subject, "fixed-point construction cases accepted")]
    return [
        _rejected(
            subject,
            "fixed-point construction cases rejected: "
            + _joined_or_none(report.failed_subjects),
        )
    ]
...
def _failed_subject_for_result(subject: str) -> str:
    ...
    if subject.endswith(".substitution_graph_correctness_cases"):
