"""Evidence bundles for post-handoff network sequence witnesses.

Network sequence evidence bundles sit above the ADR-0197 claim surface. They
do not add a new Universal Cell transition or a general scheduler; they make
one post-handoff sequence auditable across its sequence claim, proof
certificate, executable witness, trace, SVG, underlying delivery chain bundle,
and source-status boundaries.
"""

from __future__ import annotations

import argparse
import json
from dataclasses import dataclass
from pathlib import Path
from typing import Any

from autarkic_systems import network_sequence_predicates
from autarkic_systems.chain_evidence_bundle import (
    load_transition_chain_evidence_bundle,
    validate_transition_chain_evidence_bundle,
)
from autarkic_systems.network_sequence import execute_post_handoff_signal_witness
from autarkic_systems.network_sequence_claims import (
    NetworkSequenceClaim,
    NetworkSequenceClaimExample,
    load_network_sequence_claims,
    validate_network_sequence_claim_project,
)
from autarkic_systems.network_sequence_object_language import (
    validate_network_sequence_claim_language_project,
)
from autarkic_systems.network_sequence_trace import (
    load_network_sequence_trace,
    validate_network_sequence_trace,
)
from autarkic_systems.network_sequence_svg import validate_network_sequence_svg


@dataclass(frozen=True)
class NetworkSequenceEvidenceBundle:
    """One inspectable path from sequence behavior to supporting evidence."""

    schema_version: int
    bundle_id: str
    reviewed_at: str
    purpose: str
    sequence_claim_id: str
    predicate: str
    positive_example: str
    sequence_function: str
    expected_status: str
    sequence_claim_manifest_path: Path
    sequence_proof_certificate_path: Path
    sequence_language_path: Path
    sequence_claim_validator_path: Path
    sequence_witness_path: Path
    sequence_trace_path: Path
    sequence_svg_path: Path
    chain_bundle_paths: tuple[Path, ...]
    source_status_paths: tuple[Path, ...]
    boundaries: tuple[str, ...]


@dataclass(frozen=True)
class NetworkSequenceEvidenceBundleValidation:
    """One validation result for a network-sequence evidence bundle."""

    subject: str
    accepted: bool
    detail: str


@dataclass(frozen=True)
class NetworkSequenceEvidenceBundleRegistryEntry:
    """One registered network-sequence evidence bundle."""

    bundle_id: str
    path: Path
    sequence_claim_id: str
    expected_status: str


@dataclass(frozen=True)
class NetworkSequenceEvidenceBundleRegistry:
    """Project-level index of network-sequence evidence bundles."""

    schema_version: int
    registry_id: str
    reviewed_at: str
    purpose: str
    bundles: tuple[NetworkSequenceEvidenceBundleRegistryEntry, ...]
    source_path: Path | None = None


def load_network_sequence_evidence_bundle(
    path: Path | str,
) -> NetworkSequenceEvidenceBundle:
    """Load a network-sequence evidence bundle from JSON."""

    data = json.loads(Path(path).read_text(encoding="utf-8"))
    artifacts = _required_dict(data, "artifacts")
    return NetworkSequenceEvidenceBundle(
        schema_version=_required_int(data, "schema_version"),
        bundle_id=_required_text(data, "bundle_id"),
        reviewed_at=_required_text(data, "reviewed_at"),
        purpose=_required_text(data, "purpose"),
        sequence_claim_id=_required_text(data, "sequence_claim_id"),
        predicate=_required_text(data, "predicate"),
        positive_example=_required_text(data, "positive_example"),
        sequence_function=_required_text(data, "sequence_function"),
        expected_status=_required_text(data, "expected_status"),
        sequence_claim_manifest_path=Path(
            _required_text(artifacts, "sequence_claims")
        ),
        sequence_proof_certificate_path=Path(
            _required_text(artifacts, "sequence_proof_certificates")
        ),
        sequence_language_path=Path(_required_text(artifacts, "sequence_language")),
        sequence_claim_validator_path=Path(
            _required_text(artifacts, "sequence_claim_validator")
        ),
        sequence_witness_path=Path(_required_text(artifacts, "sequence_witness")),
        sequence_trace_path=Path(_required_text(artifacts, "sequence_trace")),
        sequence_svg_path=Path(_required_text(artifacts, "sequence_svg")),
        chain_bundle_paths=tuple(
            Path(path) for path in _required_text_list(artifacts, "chain_bundles")
        ),
        source_status_paths=tuple(
            Path(path) for path in _required_text_list(artifacts, "source_statuses")
        ),
        boundaries=tuple(_required_text_list(data, "boundaries")),
    )


def load_network_sequence_evidence_bundle_registry(
    path: Path | str,
) -> NetworkSequenceEvidenceBundleRegistry:
    """Load the project network-sequence evidence bundle registry."""

    registry_path = Path(path)
    data = json.loads(registry_path.read_text(encoding="utf-8"))
    bundles = data.get("bundles")
    if not isinstance(bundles, list) or not bundles:
        raise ValueError(
            "network sequence evidence registry must contain a non-empty bundles list"
        )
    return NetworkSequenceEvidenceBundleRegistry(
        schema_version=_required_int(data, "schema_version"),
        registry_id=_required_text(data, "registry_id"),
        reviewed_at=_required_text(data, "reviewed_at"),
        purpose=_required_text(data, "purpose"),
        bundles=tuple(_parse_registry_entry(entry) for entry in bundles),
        source_path=registry_path,
    )


def validate_network_sequence_evidence_bundle(
    bundle: NetworkSequenceEvidenceBundle,
) -> list[NetworkSequenceEvidenceBundleValidation]:
    """Validate a sequence bundle across claim, witness, and evidence layers."""

    results = [_validate_schema(bundle)]
    _claim, _example, claim_result = _validate_sequence_claim_example(bundle)
    results.append(claim_result)
    results.append(_validate_sequence_proof_certificate(bundle))
    results.append(_validate_sequence_language(bundle))
    results.append(_validate_sequence_witness(bundle))
    results.append(_validate_sequence_trace(bundle))
    results.append(_validate_sequence_svg(bundle))
    results.append(_validate_underlying_chain_bundles(bundle))
    results.append(_validate_source_statuses(bundle))
    results.append(_validate_boundary(bundle))
    return results


def validate_network_sequence_evidence_bundle_registry(
    registry: NetworkSequenceEvidenceBundleRegistry,
) -> list[NetworkSequenceEvidenceBundleValidation]:
    """Validate every network-sequence bundle listed in a registry."""

    return [
        _validate_registry_schema(registry),
        _validate_registry_entries(registry),
        _validate_registry_bundle_paths(registry),
        _validate_registry_bundles(registry),
        _validate_registry_completeness(registry),
    ]


def format_network_sequence_evidence_bundle_report(
    bundle: NetworkSequenceEvidenceBundle,
    results: list[NetworkSequenceEvidenceBundleValidation],
) -> str:
    """Format a concise operator report for sequence bundle validation."""

    lines = [f"Network sequence evidence bundle: {bundle.bundle_id}"]
    for result in results:
        prefix = "OK" if result.accepted else "FAIL"
        lines.append(f"{prefix} {result.subject}: {result.detail}")
    return "\n".join(lines)


def format_network_sequence_registry_validation_report(
    registry: NetworkSequenceEvidenceBundleRegistry,
    results: list[NetworkSequenceEvidenceBundleValidation],
) -> str:
    """Format a concise operator report for sequence registry validation."""

    lines = [f"Network sequence evidence registry: {registry.registry_id}"]
    for result in results:
        prefix = "OK" if result.accepted else "FAIL"
        lines.append(f"{prefix} {result.subject}: {result.detail}")
    return "\n".join(lines)


def network_sequence_evidence_bundle_report_payload(
    bundle: NetworkSequenceEvidenceBundle,
    results: list[NetworkSequenceEvidenceBundleValidation],
) -> dict[str, Any]:
    """Return a structured sequence bundle validation payload."""

    return {
        "bundle_id": bundle.bundle_id,
        "accepted": all(result.accepted for result in results),
        "sequence_claim_id": bundle.sequence_claim_id,
        "failed_subjects": [
            result.subject for result in results if not result.accepted
        ],
        "result_count": len(results),
        "results": [
            {
                "subject": result.subject,
                "accepted": result.accepted,
                "detail": result.detail,
            }
            for result in results
        ],
    }


def network_sequence_registry_validation_report_payload(
    registry: NetworkSequenceEvidenceBundleRegistry,
    results: list[NetworkSequenceEvidenceBundleValidation],
) -> dict[str, Any]:
    """Return a structured sequence registry validation payload."""

    return {
        "registry_id": registry.registry_id,
        "accepted": all(result.accepted for result in results),
        "bundle_count": len(registry.bundles),
        "failed_subjects": [
            result.subject for result in results if not result.accepted
        ],
        "bundle_failed_subjects": _registry_bundle_failed_subjects(registry),
        "bundles": [
            {
                "bundle_id": entry.bundle_id,
                "path": str(entry.path),
                "sequence_claim_id": entry.sequence_claim_id,
                "expected_status": entry.expected_status,
            }
            for entry in registry.bundles
        ],
        "result_count": len(results),
        "results": [
            {
                "subject": result.subject,
                "accepted": result.accepted,
                "detail": result.detail,
            }
            for result in results
        ],
    }


def run_network_sequence_evidence_bundle_cli(argv: list[str] | None = None) -> int:
    """Run network-sequence evidence bundle validation."""

    parser = argparse.ArgumentParser(
        prog="python -m autarkic_systems.network_sequence_evidence_bundle",
        description="Validate an AS network-sequence evidence bundle.",
    )
    target_group = parser.add_mutually_exclusive_group()
    target_group.add_argument(
        "--bundle",
        default=None,
        help="Path to the network-sequence evidence bundle JSON.",
    )
    target_group.add_argument(
        "--registry",
        default=None,
        help="Path to a network-sequence evidence bundle registry JSON.",
    )
    parser.add_argument(
        "--format",
        choices=("text", "json"),
        default="text",
        help="Output format for the validation report.",
    )
    args = parser.parse_args(argv)

    if args.registry is not None:
        registry = load_network_sequence_evidence_bundle_registry(args.registry)
        results = validate_network_sequence_evidence_bundle_registry(registry)
        if args.format == "json":
            payload = network_sequence_registry_validation_report_payload(
                registry,
                results,
            )
            print(json.dumps(payload, sort_keys=True))
        else:
            print(format_network_sequence_registry_validation_report(registry, results))
        return 0 if all(result.accepted for result in results) else 1

    bundle_path = args.bundle or "evidence/sequences/post_handoff_signal_bundle.json"
    bundle = load_network_sequence_evidence_bundle(bundle_path)
    results = validate_network_sequence_evidence_bundle(bundle)
    if args.format == "json":
        payload = network_sequence_evidence_bundle_report_payload(bundle, results)
        print(json.dumps(payload, sort_keys=True))
    else:
        print(format_network_sequence_evidence_bundle_report(bundle, results))
    return 0 if all(result.accepted for result in results) else 1


def _validate_schema(
    bundle: NetworkSequenceEvidenceBundle,
) -> NetworkSequenceEvidenceBundleValidation:
    if bundle.schema_version != 1:
        return _rejected("schema", f"unsupported schema version {bundle.schema_version}")
    required_paths = (
        bundle.sequence_claim_manifest_path,
        bundle.sequence_proof_certificate_path,
        bundle.sequence_language_path,
        bundle.sequence_claim_validator_path,
        bundle.sequence_witness_path,
        bundle.sequence_trace_path,
        bundle.sequence_svg_path,
        *bundle.chain_bundle_paths,
        *bundle.source_status_paths,
    )
    missing = [str(path) for path in required_paths if not path.exists()]
    if missing:
        return _rejected("schema", f"missing artifact paths: {', '.join(missing)}")
    return _accepted("schema", "bundle schema accepted")


def _validate_sequence_claim_example(
    bundle: NetworkSequenceEvidenceBundle,
) -> tuple[
    NetworkSequenceClaim | None,
    NetworkSequenceClaimExample | None,
    NetworkSequenceEvidenceBundleValidation,
]:
    try:
        claim, example = _bundle_claim_example(bundle)
    except Exception as exc:
        return None, None, _rejected("sequence-claim-example", str(exc))

    if claim.predicate != bundle.predicate:
        return (
            claim,
            example,
            _rejected(
                "sequence-claim-example",
                f"predicate mismatch: {claim.predicate} != {bundle.predicate}",
            ),
        )
    if example.expected is not True:
        return (
            claim,
            example,
            _rejected("sequence-claim-example", "positive example is not expected true"),
        )
    checker = getattr(network_sequence_predicates, bundle.predicate, None)
    if checker is None:
        return (
            claim,
            example,
            _rejected(
                "sequence-claim-example",
                f"unknown sequence predicate checker: {bundle.predicate}",
            ),
        )
    witness = execute_post_handoff_signal_witness(
        example.sender,
        example.recipient,
        followup_input=example.followup_input,
    )
    predicate_result = checker(witness)
    if not predicate_result.holds:
        return _claim_example_result(
            claim,
            example,
            False,
            predicate_result.detail,
        )
    return _claim_example_result(
        claim,
        example,
        True,
        f"example {example.name!r} satisfies {bundle.predicate}",
    )


def _validate_sequence_proof_certificate(
    bundle: NetworkSequenceEvidenceBundle,
) -> NetworkSequenceEvidenceBundleValidation:
    report = validate_network_sequence_claim_project(
        claims_path=bundle.sequence_claim_manifest_path,
        certificates_path=bundle.sequence_proof_certificate_path,
    )
    failures = [
        result.detail for result in report.results if not result.accepted
    ]
    if failures:
        return _rejected("sequence-proof-certificate", " | ".join(failures))
    return _accepted(
        "sequence-proof-certificate",
        f"verified {report.certificate_count} sequence certificates",
    )


def _validate_sequence_language(
    bundle: NetworkSequenceEvidenceBundle,
) -> NetworkSequenceEvidenceBundleValidation:
    try:
        report = validate_network_sequence_claim_language_project(
            language_path=bundle.sequence_language_path,
            claims_path=bundle.sequence_claim_manifest_path,
            certificates_path=bundle.sequence_proof_certificate_path,
        )
    except Exception as exc:
        return _rejected("sequence-language", f"{type(exc).__name__}: {exc}")

    failures = [
        result.detail for result in report.results if not result.accepted
    ]
    if failures:
        return _rejected("sequence-language", " | ".join(failures))
    return _accepted(
        "sequence-language",
        f"validated {report.claim_count} sequence language claims",
    )


def _validate_sequence_witness(
    bundle: NetworkSequenceEvidenceBundle,
) -> NetworkSequenceEvidenceBundleValidation:
    try:
        _claim, example = _bundle_claim_example(bundle)
    except Exception as exc:
        return _rejected("sequence-witness", str(exc))

    witness = execute_post_handoff_signal_witness(
        example.sender,
        example.recipient,
        followup_input=example.followup_input,
    )
    if witness.status != bundle.expected_status:
        return _rejected(
            "sequence-witness",
            f"status mismatch: {witness.status} != {bundle.expected_status}",
        )
    if not witness.accepted:
        return _rejected(
            "sequence-witness",
            f"witness is not accepted: {witness.detail}",
        )
    return _accepted(
        "sequence-witness",
        f"{bundle.sequence_function} produced {witness.status}",
    )


def _validate_sequence_trace(
    bundle: NetworkSequenceEvidenceBundle,
) -> NetworkSequenceEvidenceBundleValidation:
    try:
        trace = load_network_sequence_trace(bundle.sequence_trace_path)
        trace_results = validate_network_sequence_trace(trace)
    except Exception as exc:
        return _rejected("sequence-trace", f"{type(exc).__name__}: {exc}")

    failures = [
        result.detail for result in trace_results if not result.accepted
    ]
    if trace.sequence_claim_id != bundle.sequence_claim_id:
        failures.append(
            f"sequence claim mismatch: {trace.sequence_claim_id} "
            f"!= {bundle.sequence_claim_id}"
        )
    if trace.sequence_helper != bundle.sequence_function:
        failures.append(
            f"sequence helper mismatch: {trace.sequence_helper} "
            f"!= {bundle.sequence_function}"
        )
    if trace.expected_status != bundle.expected_status:
        failures.append(
            f"expected status mismatch: {trace.expected_status} "
            f"!= {bundle.expected_status}"
        )
    if failures:
        return _rejected("sequence-trace", " | ".join(failures))
    return _accepted("sequence-trace", f"validated {trace.artifact_id}")


def _validate_sequence_svg(
    bundle: NetworkSequenceEvidenceBundle,
) -> NetworkSequenceEvidenceBundleValidation:
    try:
        trace = load_network_sequence_trace(bundle.sequence_trace_path)
        svg_text = bundle.sequence_svg_path.read_text(encoding="utf-8")
        svg_results = validate_network_sequence_svg(trace, svg_text=svg_text)
    except Exception as exc:
        return _rejected("sequence-svg", f"{type(exc).__name__}: {exc}")

    failures = [
        result.detail for result in svg_results if not result.accepted
    ]
    if failures:
        return _rejected("sequence-svg", " | ".join(failures))
    return _accepted("sequence-svg", f"validated {bundle.sequence_svg_path}")


def _validate_underlying_chain_bundles(
    bundle: NetworkSequenceEvidenceBundle,
) -> NetworkSequenceEvidenceBundleValidation:
    failures: list[str] = []
    for path in bundle.chain_bundle_paths:
        try:
            chain_bundle = load_transition_chain_evidence_bundle(path)
            results = validate_transition_chain_evidence_bundle(chain_bundle)
        except Exception as exc:
            failures.append(f"{path}: {exc}")
            continue
        rejected = [result.detail for result in results if not result.accepted]
        if rejected:
            failures.append(f"{path}: {'; '.join(rejected)}")
    if failures:
        return _rejected("underlying-chain-bundles", " | ".join(failures))
    return _accepted(
        "underlying-chain-bundles",
        f"validated {len(bundle.chain_bundle_paths)} chain bundles",
    )


def _validate_source_statuses(
    bundle: NetworkSequenceEvidenceBundle,
) -> NetworkSequenceEvidenceBundleValidation:
    failures: list[str] = []
    for path in bundle.source_status_paths:
        if not path.exists():
            failures.append(f"{path}: missing")
            continue
        try:
            data = json.loads(path.read_text(encoding="utf-8"))
        except json.JSONDecodeError as exc:
            failures.append(f"{path}: invalid json: {exc}")
            continue
        if not isinstance(data, dict):
            failures.append(f"{path}: source status is not a JSON object")
    if failures:
        return _rejected("source-statuses", " | ".join(failures))
    return _accepted(
        "source-statuses",
        f"validated {len(bundle.source_status_paths)} source-status files",
    )


def _validate_boundary(
    bundle: NetworkSequenceEvidenceBundle,
) -> NetworkSequenceEvidenceBundleValidation:
    if not bundle.boundaries:
        return _rejected("boundary", "bundle must declare boundaries")
    boundary_text = " ".join(bundle.boundaries).lower()
    required_terms = ("scheduler", "topology", "new command semantics")
    missing = [term for term in required_terms if term not in boundary_text]
    if missing:
        return _rejected(
            "boundary",
            f"boundary text missing terms: {', '.join(missing)}",
        )
    return _accepted("boundary", f"recorded {len(bundle.boundaries)} boundaries")


def _validate_registry_schema(
    registry: NetworkSequenceEvidenceBundleRegistry,
) -> NetworkSequenceEvidenceBundleValidation:
    if registry.schema_version != 1:
        return _rejected(
            "registry-schema",
            f"unsupported schema version {registry.schema_version}",
        )
    if not registry.registry_id:
        return _rejected("registry-schema", "registry id is empty")
    if not registry.bundles:
        return _rejected("registry-schema", "registry has no bundles")
    return _accepted("registry-schema", "registry schema accepted")


def _validate_registry_entries(
    registry: NetworkSequenceEvidenceBundleRegistry,
) -> NetworkSequenceEvidenceBundleValidation:
    bundle_ids = [entry.bundle_id for entry in registry.bundles]
    paths = [entry.path for entry in registry.bundles]
    duplicate_ids = sorted(_duplicates(bundle_ids))
    duplicate_paths = sorted(str(path) for path in _duplicates(paths))
    details: list[str] = []
    if duplicate_ids:
        details.append(f"duplicate bundle ids: {', '.join(duplicate_ids)}")
    if duplicate_paths:
        details.append(f"duplicate bundle paths: {', '.join(duplicate_paths)}")
    if details:
        return _rejected("registry-entries", "; ".join(details))
    return _accepted("registry-entries", f"registered {len(registry.bundles)} bundles")


def _validate_registry_bundle_paths(
    registry: NetworkSequenceEvidenceBundleRegistry,
) -> NetworkSequenceEvidenceBundleValidation:
    missing = [str(entry.path) for entry in registry.bundles if not entry.path.exists()]
    if missing:
        return _rejected("registry-bundle-paths", f"missing bundle: {', '.join(missing)}")
    return _accepted("registry-bundle-paths", "all registered bundle paths exist")


def _validate_registry_bundles(
    registry: NetworkSequenceEvidenceBundleRegistry,
) -> NetworkSequenceEvidenceBundleValidation:
    failures: list[str] = []
    for entry in registry.bundles:
        try:
            bundle = load_network_sequence_evidence_bundle(entry.path)
        except Exception as exc:
            failures.append(f"{entry.path}: {exc}")
            continue
        if bundle.bundle_id != entry.bundle_id:
            failures.append(f"{entry.path}: bundle id mismatch {bundle.bundle_id}")
        if bundle.sequence_claim_id != entry.sequence_claim_id:
            failures.append(
                f"{entry.path}: sequence claim id mismatch {bundle.sequence_claim_id}"
            )
        if bundle.expected_status != entry.expected_status:
            failures.append(f"{entry.path}: status mismatch {bundle.expected_status}")
        bundle_results = validate_network_sequence_evidence_bundle(bundle)
        rejected = [result.detail for result in bundle_results if not result.accepted]
        if rejected:
            failures.append(f"{entry.path}: {'; '.join(rejected)}")
    if failures:
        return _rejected("registry-bundle-validation", " | ".join(failures))
    return _accepted(
        "registry-bundle-validation",
        f"validated {len(registry.bundles)} bundles",
    )


def _registry_bundle_failed_subjects(
    registry: NetworkSequenceEvidenceBundleRegistry,
) -> list[dict[str, Any]]:
    failed: list[dict[str, Any]] = []
    for entry in registry.bundles:
        if not entry.path.exists():
            continue
        try:
            bundle = load_network_sequence_evidence_bundle(entry.path)
        except Exception:
            continue
        results = validate_network_sequence_evidence_bundle(bundle)
        failed_subjects = [
            result.subject for result in results if not result.accepted
        ]
        if failed_subjects:
            failed.append({
                "bundle_id": entry.bundle_id,
                "failed_subjects": failed_subjects,
            })
    return failed


def _validate_registry_completeness(
    registry: NetworkSequenceEvidenceBundleRegistry,
) -> NetworkSequenceEvidenceBundleValidation:
    registry_dir = _registry_directory(registry)
    registered = {_normalized_path(entry.path) for entry in registry.bundles}
    discovered = {
        _normalized_path(path)
        for path in sorted(registry_dir.glob("*_bundle.json"))
    }
    unregistered = sorted(discovered - registered)
    if unregistered:
        display = ", ".join(str(path) for path in unregistered)
        return _rejected(
            "registry-completeness",
            f"unregistered bundle files: {display}",
        )
    return _accepted(
        "registry-completeness",
        f"all {len(discovered)} discovered bundle files are registered",
    )


def _bundle_claim_example(
    bundle: NetworkSequenceEvidenceBundle,
) -> tuple[NetworkSequenceClaim, NetworkSequenceClaimExample]:
    claims = load_network_sequence_claims(bundle.sequence_claim_manifest_path)
    matching_claims = [
        claim for claim in claims if claim.claim_id == bundle.sequence_claim_id
    ]
    if not matching_claims:
        raise ValueError(f"missing sequence claim {bundle.sequence_claim_id}")
    if len(matching_claims) > 1:
        raise ValueError(f"duplicate sequence claim {bundle.sequence_claim_id}")
    claim = matching_claims[0]
    matching_examples = [
        example for example in claim.examples if example.name == bundle.positive_example
    ]
    if not matching_examples:
        raise ValueError(
            f"missing positive example {bundle.positive_example!r}"
        )
    if len(matching_examples) > 1:
        raise ValueError(
            f"duplicate positive example {bundle.positive_example!r}"
        )
    return claim, matching_examples[0]


def _claim_example_result(
    claim: NetworkSequenceClaim,
    example: NetworkSequenceClaimExample,
    accepted: bool,
    detail: str,
) -> tuple[
    NetworkSequenceClaim,
    NetworkSequenceClaimExample,
    NetworkSequenceEvidenceBundleValidation,
]:
    result = _accepted("sequence-claim-example", detail)
    if not accepted:
        result = _rejected("sequence-claim-example", detail)
    return claim, example, result


def _parse_registry_entry(
    item: dict[str, Any],
) -> NetworkSequenceEvidenceBundleRegistryEntry:
    return NetworkSequenceEvidenceBundleRegistryEntry(
        bundle_id=_required_text(item, "bundle_id"),
        path=Path(_required_text(item, "path")),
        sequence_claim_id=_required_text(item, "sequence_claim_id"),
        expected_status=_required_text(item, "expected_status"),
    )


def _registry_directory(registry: NetworkSequenceEvidenceBundleRegistry) -> Path:
    if registry.source_path is None:
        return Path("evidence/sequences")
    return registry.source_path.parent


def _normalized_path(path: Path) -> Path:
    try:
        return path.resolve()
    except FileNotFoundError:
        return path


def _duplicates(values: list[Any]) -> set[Any]:
    seen: set[Any] = set()
    duplicates: set[Any] = set()
    for value in values:
        if value in seen:
            duplicates.add(value)
        seen.add(value)
    return duplicates


def _required_dict(item: dict[str, Any], key: str) -> dict[str, Any]:
    value = item.get(key)
    if not isinstance(value, dict):
        raise ValueError(f"required object field missing: {key}")
    return value


def _required_int(item: dict[str, Any], key: str) -> int:
    value = item.get(key)
    if not isinstance(value, int):
        raise ValueError(f"required integer field missing: {key}")
    return value


def _required_text(item: dict[str, Any], key: str) -> str:
    value = item.get(key)
    if not isinstance(value, str) or not value:
        raise ValueError(f"required text field missing: {key}")
    return value


def _required_text_list(item: dict[str, Any], key: str) -> list[str]:
    value = item.get(key)
    if not isinstance(value, list) or not value:
        raise ValueError(f"required text list field missing: {key}")
    for entry in value:
        if not isinstance(entry, str) or not entry:
            raise ValueError(f"{key} contains non-text entry")
    return value


def _accepted(
    subject: str,
    detail: str,
) -> NetworkSequenceEvidenceBundleValidation:
    return NetworkSequenceEvidenceBundleValidation(subject, True, detail)


def _rejected(
    subject: str,
    detail: str,
) -> NetworkSequenceEvidenceBundleValidation:
    return NetworkSequenceEvidenceBundleValidation(subject, False, detail)


if __name__ == "__main__":
    raise SystemExit(run_network_sequence_evidence_bundle_cli())
