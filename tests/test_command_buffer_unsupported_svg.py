...
from autarkic_systems.schematic_svg import (
    COMMAND_BUFFER_UNSUPPORTED_SVG_ARTIFACT,
    SVG_NAMESPACE,
...
TRACE_ARTIFACT = Path("schematics/command_buffer_unsupported_trace.json")


class CommandBufferUnsupportedSvgTests(unittest.TestCase):
    def setUp(self):
        self.trace = load_schematic_trace(TRACE_ARTIFACT)
        self.svg_text = render_schematic_svg(self.trace)

    def test_svg_is_nonblank_xml_with_unsupported_command_buffer_metadata(self):
    ...
    def test_svg_labels_unsupported_command_buffer_ports_from_trace(self):
        root = ET.fromstring(self.svg_text)
    ...
    def test_svg_records_unsupported_command_buffer_details_and_flow(self):
        ...
        self.assertIn("status: stem-buffer-appended", self.svg_text)
        ...
        self.assertIn("control before: [0, 0, 1]", self.svg_text)
        self.assertIn("buffer before: [0, 1, 0, 0]", self.svg_text)
        self.assertIn("input after: [_, _, _]", self.svg_text)
        self.assertIn("control after: [0, 0, 1]", self.svg_text)
        self.assertIn("buffer after: [0, 1, 0, 0, 1]", self.svg_text)
        root = ET.fromstring(self.svg_text)
        visible_text = "\n".join(root.itertext())
    ...
    def test_svg_exposes_unsupported_command_buffer_interpretive_layer_ids(self):
        root = ET.fromstring(self.svg_text)
    ...
    def test_committed_unsupported_command_buffer_svg_matches_renderer_output(self):
        committed = COMMAND_BUFFER_UNSUPPORTED_SVG_ARTIFACT.read_text(
            encoding="utf-8",
        )

        self.assertEqual(committed, self.svg_text)

    def test_unsupported_command_buffer_svg_validator_accepts_committed_svg(self):
        results = validate_schematic_svg(
            self.trace,
            svg_text=COMMAND_BUFFER_UNSUPPORTED_SVG_ARTIFACT.read_text(
                encoding="utf-8",
            ),
        )
    ...
    def test_unsupported_command_buffer_svg_validator_rejects_drifted_buffer(self):
        drifted = self.svg_text.replace(
            "buffer after: [0, 1, 0, 0, 1]",
            "buffer after: [0, 1, 0, 0, 0]",
        )
