...
from autarkic_systems.schematic_svg import (
    RECIPIENT_INIT_COMMAND_MESSAGE_SVG_ARTIFACT,
    SVG_NAMESPACE,
...
TRACE_ARTIFACT = Path("schematics/recipient_init_command_message_trace.json")


class RecipientInitCommandMessageSvgTests(unittest.TestCase):
    def setUp(self):
        self.trace = load_schematic_trace(TRACE_ARTIFACT)
        self.svg_text = render_schematic_svg(self.trace)

    def test_svg_is_nonblank_xml_with_recipient_trace_metadata(self):
    ...
    def test_svg_labels_recipient_ports_from_trace(self):
        root = ET.fromstring(self.svg_text)
    ...
    def test_svg_records_recipient_command_details_and_flow(self):
        ...
        self.assertIn("status: recipient-init-command-message-processed", self.svg_text)
        self.assertIn("transition: step_fixed_cell", self.svg_text)
        self.assertIn("upstream before: [wire-r-init, _, _]", self.svg_text)
    ...
    def test_svg_exposes_recipient_interpretive_layer_ids(self):
        root = ET.fromstring(self.svg_text)
    ...
    def test_committed_recipient_svg_matches_renderer_output(self):
        committed = RECIPIENT_INIT_COMMAND_MESSAGE_SVG_ARTIFACT.read_text(
            encoding="utf-8",
        )

        self.assertEqual(committed, self.svg_text)

    def test_recipient_svg_validator_accepts_committed_svg(self):
        results = validate_schematic_svg(
            self.trace,
            svg_text=RECIPIENT_INIT_COMMAND_MESSAGE_SVG_ARTIFACT.read_text(
                encoding="utf-8",
            ),
        )
    ...
    def test_recipient_svg_validator_rejects_drifted_upstream(self):
        drifted = self.svg_text.replace(
            "upstream after: [_, _, _]",
            "upstream after: [wire-r-init, _, _]",
        )
