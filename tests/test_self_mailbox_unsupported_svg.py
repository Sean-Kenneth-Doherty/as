...
class SelfMailboxUnsupportedSvgTests(unittest.TestCase):
    ...
    def test_svg_records_unsupported_mailbox_preservation_and_flow(self):
        ...
        self.assertIn("control before: [1, 1, 0]", self.svg_text)
        self.assertIn("control after: [1, 1, 0]", self.svg_text)
        self.assertIn("buffer before: [1, 0, 1]", self.svg_text)
        self.assertIn("buffer after: [1, 0, 1]", self.svg_text)
