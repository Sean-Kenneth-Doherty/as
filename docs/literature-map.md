...
| AS recipient command consumption source status | `docs/recipient-command-consumption-source-status.md` and `sources/recipient_command_consumption_source_status.json` | Source-status decision for recipient-side command-message inputs after neighbor delivery. | Permits only init-family recipient consumption next; keeps `standard-signal`, write-buffer, and multi-command conflict policy blocked. |
...
| Recipient-side command-message consumption needs a source-status decision before runtime behavior changes. | `sources/recipient_command_consumption_source_status.json` and restored PRC witnesses under `/home/sean/Projects/_upstream/prc`. | Implemented in ADR-0048; next executable slice is init-family recipient command-message consumption only. |
...
- AS has rendered the wire, processor, stem automail, and stem buffer
  ...
  slice to init-family command-message inputs.
