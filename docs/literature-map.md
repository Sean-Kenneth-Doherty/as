...
| AS write-buffer command semantics status | `docs/write-buffer-command-semantics-status.md` and `sources/write_buffer_command_semantics_status.json` | Source-status decision for `write-buf-zero` and `write-buf-one` execution. | Keeps write-buffer execution blocked because formal, RAA, SEMSIM, and FSMSIM witnesses do not agree on append, clearing, and buffer-full boundaries. |
| AS standard-signal command semantics status | `docs/standard-signal-command-semantics-status.md` and `sources/standard_signal_command_semantics_status.json` | Source-status decision for `standard-signal` command-token execution. | Keeps command-token execution blocked because the formal command table, ordinary binary-input standard-signal behavior, and legacy special-message sketches do not settle the same runtime surface. |
...
- AS has rendered the wire, processor, stem automail, and stem buffer
  ...
  render for that trace. ADR-0048 records the source-status boundary for
