...
| AFS-R8 | Bridge formal claims to substrate claims through named predicates and proof obligations. | AS must integrate SJAS formal confidence with PRC substrate visibility. | A small toy bridge: a proof-side predicate about a transition-system invariant, checked against executable substrate examples. | Partially started on the substrate side; formal-side predicates remain open. |
