# ADR-0181: Command-Buffer Unsupported Predicate Result Certificates

Date: 2026-05-18

## Status

Accepted.

## Context

ADR-0172 through ADR-0180 progressively migrated early transition proof
certificates from implicit `manifest-example` steps to explicit
`predicate-result` steps. The next transition certificate in the manifest,
`UC-STEM-COMMAND-BUFFER-UNSUPPORTED-APPENDED`, still uses `manifest-example`
proof steps even though the object language already names its predicate as
`stem_command_buffer_preserves_unsupported_completion`.

This claim is the completed self-target `standard-signal` command-buffer
append-preservation boundary. ADR-0161 moved self-target write-buffer command
buffers into explicit append execution claims, so this slice also updates the
older claim note to match the current narrowed standard-signal-only boundary.
Migrating its certificate continues the proof-object hardening path without
changing runtime behavior, transition claims, object-language schema, evidence
bundles, source-status records, or project-status schema versions.

## Decision

Migrate `UC-STEM-COMMAND-BUFFER-UNSUPPORTED-APPENDED` from
`manifest-example` proof steps to `predicate-result` proof steps that
explicitly name `stem_command_buffer_preserves_unsupported_completion`.

## Success Criteria

- Red tests fail before implementation because
  `UC-STEM-COMMAND-BUFFER-UNSUPPORTED-APPENDED` still uses
  `manifest-example`.
- The command-buffer unsupported certificate has exactly two
  `predicate-result` steps.
- Both command-buffer unsupported steps name
  `stem_command_buffer_preserves_unsupported_completion`.
- Proof-certificate text/JSON and aggregate project-status output report the
  command-buffer unsupported certificate as two `predicate-result` steps.
- The command-buffer unsupported claim note matches the current
  standard-signal-only boundary.
- Full repository tests remain green.

## Test Plan

- Red:
  `python -m unittest tests.test_proof_certificates tests.test_project_status_report`
  fails before the proof-certificate manifest update.
- Green: the same focused suite passes after updating
  `claims/proof_certificates.json` and expected status output.
- Regression: run JSON parsing for touched JSON, proof-certificate CLI JSON,
  project-status JSON, object-language JSON, `compileall`, `git diff --check`,
  and the full default suite before commit.

## After Action Report

Implemented. The focused red run
`python -m unittest tests.test_proof_certificates tests.test_project_status_report`
ran 99 tests and failed because the command-buffer unsupported certificate
still used `manifest-example`, and proof-certificate/project-status reports
still rendered the command-buffer unsupported certificate as two
`manifest-example` steps.

The implementation migrated both
`UC-STEM-COMMAND-BUFFER-UNSUPPORTED-APPENDED` certificate steps to
`predicate-result` and named
`stem_command_buffer_preserves_unsupported_completion` directly on both steps.
It also refreshed the command-buffer unsupported claim note so it no longer
states that self-target write-buffer completions remain inside this
unsupported boundary after ADR-0161. No runtime behavior, transition claims,
object-language rules, evidence bundles, source-status records, or status
schema versions changed.

The same focused suite then passed with 99 tests. The proof-certificate CLI
JSON, project-status JSON, and object-language JSON checks accepted the updated
certificate surface. JSON parsing for the touched certificate manifest,
`compileall`, `git diff --check`, and the full default suite also passed; the
full suite ran 779 tests.
